    ...
    with col2:
        ...
        if not sdf.empty:
            for _, row in sdf.iterrows():
                st.markdown(f"<div class='premium-card'><h3 style='color:#4f46e5; margin-bottom:5px;'>{row['name']}</h3><p style='margin:2px 0;'><b>Phone:</b> {row['phone'] if row['phone'] else 'N/A'}</p><p style='margin:2px 0;'><b>Address:</b> {row['address'] if row['address'] else 'N/A'}</p></div>", unsafe_with_html=True)
                ...
                if not tdf.empty:
                    ...
                    for i, t_row in tdf.iterrows():
                        with cols[i % 4]:
                            st.markdown(f"<div style='background:#ffffff; padding:10px; border-radius:8px; border:1px solid #cbd5e1; text-align:center;'><span class='badge-buy'>SUPPLIED</span><h4 style='margin:8px 0 2px 0;'>Code: {t_row['thread_code']}</h4><p style='margin:0; color:#64748b;'>Qty: <b>{t_row['qty']}</b></p></div>", unsafe_with_html=True)
                else:
    ...
    with col2:
        ...
        if not cdf.empty:
            for _, row in cdf.iterrows():
                st.markdown(f"<div class='premium-card'><h3 style='color:#0ea5e9; margin-bottom:5px;'>{row['name']}</h3><p style='margin:2px 0;'><b>Phone:</b> {row['phone'] if row['phone'] else 'N/A'}</p><p style='margin:2px 0;'><b>Address:</b> {row['address'] if row['address'] else 'N/A'}</p></div>", unsafe_with_html=True)
                ...
                if not t_c_df.empty:
                    ...
                    for i, tc_row in t_c_df.iterrows():
                        with cols[i % 4]:
                            st.markdown(f"<div style='background:#ffffff; padding:10px; border-radius:8px; border:1px solid #cbd5e1; text-align:center;'><span class='badge-sell'>BOUGHT</span><h4 style='margin:8px 0 2px 0;'>Code: {tc_row['thread_code']}</h4><p style='margin:0; color:#64748b;'>Qty: <b>{tc_row['qty']}</b></p></div>", unsafe_with_html=True)
                else:
